...
**Date:** 2026-07-20
**Supersedes:** nothing
**Amended by:** [ADR 002](./002-implementation-gaps.md) — gaps in D2/D4/D5/D9 found while
implementing, and one withdrawn claim in D1. Every decision below stands; read 002 alongside
D9 in particular, whose failure taxonomy it extends.
**Context document:** [`000-prd.md`](./000-prd.md)
**Project name:** `alertthread` (was codenamed `sturdy-telegram`; see [D12](#d12-project-name-license-and-open-source-posture) and Amendments)
...
```
alertthread_alerts_received_total{status}
alertthread_slack_calls_total{method,outcome}
alertthread_slack_call_duration_seconds{method}     histogram
alertthread_outbox_depth{op}
alertthread_outbox_oldest_age_seconds               ← primary SLO signal
alertthread_tracked_fingerprints
alertthread_orphan_resolves_total                   ← state was lost
alertthread_fallback_posts_total{reason}            ← degraded but not silent
alertthread_dead_letter_total{reason}               ← SILENT: page on this
alertthread_rate_limited_total{method}
```
...
requiring an Alertmanager route that sends `job=alertthread` alerts to a **direct Slack
...
> **Resolved 2026-07-21 — `alertthread`.** The discovery argument won, and the rename
> happened well inside the "free" window: no release was tagged and no image published.
> The reasoning above is preserved as written, because *why* the name was reconsidered is
> the part worth keeping. See Amendments.
...
1. ~~**Project name** — keep `sturdy-telegram` or rename to something descriptive. Free
   until the first tagged release, expensive after (D12).~~
   **Resolved 2026-07-21: renamed to `alertthread`.**
...
Corrections applied after this ADR was accepted. Decisions, rationale, alternatives and
consequences are never rewritten — those are superseded by a later ADR. This section is for
factual drift: names, paths, links. Each entry says what changed and why it was not a
decision.

### 2026-07-21 — codename `sturdy-telegram` replaced with `alertthread`

The project was renamed before any release was tagged or image published. Updated in place:

- the metric names in [D11](#d11-observability), `sturdy_telegram_*` → `alertthread_*`
- the example service URL in [D8](#d8-channel-routing-prd-87)
- the `job=` label in D11's circular-dependency warning
- the document header

**Why this was a correction and not a decision reversal.** No decision was ever taken that
the metric prefix should be `sturdy_telegram_`. D12 recorded the name as *provisional* and
said renaming was free until first release; those strings were mechanical renderings of a
value the ADR itself flagged as unsettled. Fixing them completes the decision D12 described
rather than overturning it.

D12's reasoning about the naming trade-off is left exactly as written, with a resolution
note appended — *why* the name was reconsidered is the part with lasting value, and the
rename decision itself is recorded in `ROADMAP.md`.

### 2026-07-21 — D1's compile-time-checked SQL claim withdrawn

D1 lists `sqlx` compile-time-checked queries against both backends as a reason to choose
Rust. Phase 2 established that this is unattainable for a dual-backend crate: `query!`
validates against one live database and one `.sqlx` cache per crate.

**The claim is withdrawn; the decision is not.** Rust stands on D1's other stated grounds.
D1's text is left as written — it records what was believed at the time — and the correction,
with its compensating control, is in [ADR 002 Part 4](./002-implementation-gaps.md#part-4--correction-to-d1).

Noted here rather than edited into D1 because this is a claim inside a decision's rationale,
not the mechanical drift this section otherwise covers. A reader of D1 needs the pointer.
